...
class MenuConsola:
    OPCIONES = """
Sistema de Gestión de Proformas - Fase 5 (SQLite)
...
""".strip()
    ...
    def __init__(
        ...
        self._acciones: dict[str, Callable[[], None]] = {
            ...
            "8": self._buscar_productos,
            "9": self._buscar_clientes,
        }
    ...
    def _buscar_productos(self) -> None:
        productos = self.aplicacion.buscar_productos(self.entrada("Texto a buscar: "))
        self.salida("\n".join(map(formatear_producto, productos)) or "No hay coincidencias.")

    def _buscar_clientes(self) -> None:
        clientes = self.aplicacion.buscar_clientes(self.entrada("Texto a buscar: "))
        self.salida("\n".join(map(formatear_cliente, clientes)) or "No hay coincidencias.")
